...
def DistributeBouns(ctx, address, start_height, end_height, amount):
    pass
...
def DoDeploy(ctx, seas, begin_bouns, bouns_interval, accept_coin):
    """
    ...
    Put(ctx, "PROFIT", 0)
    Put(ctx, "SEAS", seas)
    Put(ctx, "BEGINBOUNS", begin_bouns)
    Put(ctx, "BOUNSINTERVAL", bouns_interval)
